...
class Session:
    def __init__(self, dt_split):
        ...
        self.net = DataParallel(self.net, device_ids=[0])
    ...
    def train_batch(self, image, label):
        ...
        # 对每一个mini-batch进行moving average，防止梯度弥散，由于这个mu来自于整个mini-batch
        # 用了一个批次的平均值就会让这个值的更新变小，再使用动量的方法进行更新保证梯度的存在
        """
        with torch.no_grad():
...
def main(ckp_name='latest.pth'):
    ...
    sess = Session(dt_split='line_detect')
